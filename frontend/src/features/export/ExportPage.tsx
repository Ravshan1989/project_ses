...
import { Card, Typography, DatePicker, Select, Button, message, Row, Col } from 'antd';
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            if (reportType === 'hepatitis') {
                const res = await exportsApi.getHepatitis(startDate, endDate);
                data = res.data;
                fileName = `VGA_Report_${startDate}_${endDate}`;
            } else if (reportType === 'flu') {
                const res = await exportsApi.getFlu(startDate, endDate);
                data = res.data;
                fileName = `Flu_Report_${startDate}_${endDate}`;
            } else if (reportType === 'form1') {
                const res = await exportsApi.getForm1(startDate, endDate);
                data = res.data;
                fileName = `Form1_Report_${startDate}_${endDate}`;
            }
            ...
            <Card>
                ...
                <Row gutter={[16, 16]} style={{ marginTop: '24px' }}>
                    ...
                    <Col xs={24} md={8} style={{ display: 'flex', alignItems: 'flex-end' }}>
                        <Button type="primary" icon={<DownloadOutlined />} size="large" loading={loading} onClick={handleExport} block> Yuklab olish </Button>
                    </Col>
                </Row>
            </Card>
        </div>
