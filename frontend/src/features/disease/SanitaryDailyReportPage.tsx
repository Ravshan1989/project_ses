...
import SanitaryTab, { SanitaryReportData } from './unified/SanitaryTab';

const SanitaryDailyReportPage: React.FC = () => {
    const { t } = useTranslation();
    const [date, setDate] = useState(dayjs());
    const [data, setData] = useState<SanitaryReportData[]>([]);
    ...
    const fetchReports = async () => {
        ...
        try {
            ...
            const res = await dailyReportsApi.getSanitaryByDate(formattedDate, false);
            ...
            const isMudir = ['SANITARY_HEAD', 'DEPARTMENT_HEAD'].includes(userRole);
    ...
    const handleCellChange = (value: number | null, rowKey: string, field: keyof SanitaryReportData) => {
        const newData = [...data];
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            for (const row of data) {
                await dailyReportsApi.upsertSanitary({
                    ...row,
    ...
    const handleVerify = async (id: string) => {
        try {
            await dailyReportsApi.verify('sanitary', id);
    ...
    const handleApprove = async (id: string) => {
        try {
            await dailyReportsApi.approve('sanitary', id);
    ...
    const handleReject = async (id: string) => {
        try {
            await dailyReportsApi.reject('sanitary', id);
    ...
    const handleSubmit = async (id: string) => {
        try {
            await dailyReportsApi.submit('sanitary', id);
    ...
    return (
        <PermissionGate permission="VIEW_SANITARY">
            <GlassLayout
                title={t('daily_reports.sanitary_title') || "Sanitariya bo'limi (Kunlik)"}
...
export default SanitaryDailyReportPage;
