...
async function bootstrap() {
    ...
    if (!org) {
        ...
        const orgs = await orgRepo.createQueryBuilder("org")
    ...
    if (existing) {
        ...
        // Need to use repo directly or ensure service handles it
        const newUser = (orgRepo as any).manager.create("User", {
            ...userData,
            isActive: true
        });
        await (orgRepo as any).manager.save(newUser);
