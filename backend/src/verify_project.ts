...
async function verify() {
    try {
        ...
        if (!loginRes.ok) throw new Error("Olmaliq login failed: " + JSON.stringify(loginData));
        ...
        if (!regLoginRes.ok) throw new Error("Viloyat login failed: " + JSON.stringify(regLoginData));
        ...
        const start = today;
        const end = today;
        const summaryRes = await fetch(`${BASE_URL}/daily-reports/weekly-summary?startDate=${start}&endDate=${end}&isTest=true`, {
        ...
        const olmaliqEntry = data.find((d: any) => d.organization.name === 'Olmaliq sh');
        if (olmaliqEntry) {
            console.log("   SUCCESS: Olmaliq record found in Regional summary.");
            console.log(`   Data: ARI=${olmaliqEntry.ari_total}, Pneu=${olmaliqEntry.pneu_total}`);
        } else {
            console.error("   FAILURE: Olmaliq record NOT found in summary!");
            console.log("   Full data received counts by name:", data.map((d: any) => d.organization.name));
        }

        const otherDistricts = data.filter((d: any) => d.organization.name !== 'Olmaliq sh');
        if (otherDistricts.length > 0) {
            console.log(`   SUCCESS: ${otherDistricts.length} other districts also listed (with 0 data or reports).`);
        }
