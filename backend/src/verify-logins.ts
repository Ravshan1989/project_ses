...
async function verifyLogins() {
    ...
    const testAccounts = [
        ...
        { username: 'user_nurafshon_sh', password: 'ses12345', org: 'Nurafshon sh' },
    ...
    for (const account of testAccounts) {
        try {
            ...
            if (result.access_token && result.user) {
                console.log(`✅ ${account.org.padEnd(20)} | Login: ${account.username.padEnd(25)} | Status: SUCCESS`);
                console.log(`   Organization: ${result.user.organization?.name || 'N/A'}`);
                console.log(`   Role: ${result.user.role}\n`);
            }
        } catch (error) {
            console.log(`❌ ${account.org.padEnd(20)} | Login: ${account.username.padEnd(25)} | Status: FAILED`);
