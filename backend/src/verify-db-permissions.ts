...
async function bootstrap() {
    ...
    const permRepo: Repository<RolePermission> = app.get(getRepositoryToken(RolePermission));

    console.log("Checking permissions for DEPARTMENT_HEAD...");

    const role = await roleRepo.findOne({ where: { name: UserRole.DEPARTMENT_HEAD } });
    if (!role) {
    ...
    if (hasVerify && hasApprove) {
        ...
        console.log("Permissions list:", perms.map(p => p.permissionCode));
    }
