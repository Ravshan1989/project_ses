...
async function checkHepatitis() {
  ...
  try {
    ...
    // Hepatitis Check
    const hepatitisRepo = app.get(
      getRepositoryToken(HepatitisDailyReport),
    ) as Repository<HepatitisDailyReport>;
    ...
    // Ari Check
    const ariRepo = app.get(
      getRepositoryToken(AriDailyReport),
    ) as Repository<any>;
    ...
    orgs.forEach((o) => {
      if (
        ...
        console.log(
          `WARNING: Potential Markdown break in name: "${o.name}" (ID: ${o.id})`,
        );
      }
    });
