...
const subjects = [
  ...
  { key: "labor_relations", labelKey: "appeals.subjects.labor_relations" },
  { key: "medical_activity", labelKey: "appeals.subjects.medical_activity" },
  { key: "complaint_head", labelKey: "appeals.subjects.complaint_head" },
  { key: "staff_conduct", labelKey: "appeals.subjects.staff_conduct" },
  { key: "disinfection", labelKey: "appeals.subjects.disinfection" },
  { key: "fines_objection", labelKey: "appeals.subjects.fines_objection" },
...
export class AppealsService {
  ...
  async closeRecord(
    ...
    if (
      ...
      dayjs(closureDate).isAfter(dayjs(record.deadline_date))
    ) {
  ...
  async generateReportsFromRecords(organizationId: string, month: string) {
    ...
    const getTable1Metrics = (rec: string) => {
      ...
      return {
        ...
        oral_curr: current.filter((r) => r.channel === AppealChannel.ORAL)
          .length,
        oral_prev: previous.filter((r) => r.channel === AppealChannel.ORAL)
          .length,
        written_curr: current.filter((r) => r.channel === AppealChannel.WRITTEN)
          .length,
        written_prev: previous.filter(
          (r) => r.channel === AppealChannel.WRITTEN,
        ).length,
        electronic_curr: current.filter(
          (r) => r.channel === AppealChannel.ELECTRONIC,
        ).length,
        electronic_prev: previous.filter(
          (r) => r.channel === AppealChannel.ELECTRONIC,
        ).length,
      };
    };
    ...
    regionalOrgs.forEach((ro) => {
      const roRecs = records.filter(
        (r) =>
          r.organization?.id === ro.id ||
          ro.children?.some((c) => c.id === r.organization?.id),
      );
      const roPrevRecs = prevRecords.filter(
        (r) =>
          r.organization?.id === ro.id ||
          ro.children?.some((c) => c.id === r.organization?.id),
      );
      ...
      table3.regional[ro.id] = {
        ...
        phys_prev: roPrevRecs.filter(
          (r) => r.applicant_type === ApplicantType.PHYSICAL,
        ).length,
        phys_curr: roRecs.filter(
          (r) => r.applicant_type === ApplicantType.PHYSICAL,
        ).length,
        legal_prev: roPrevRecs.filter(
          (r) => r.applicant_type === ApplicantType.LEGAL,
        ).length,
        legal_curr: roRecs.filter(
          (r) => r.applicant_type === ApplicantType.LEGAL,
        ).length,
        written: roRecs.filter((r) => r.channel === AppealChannel.WRITTEN)
          .length,
        electronic: roRecs.filter((r) => r.channel === AppealChannel.ELECTRONIC)
          .length,
        oral_total: roRecs.filter((r) => r.channel === AppealChannel.ORAL)
          .length,
        oral_personal: roRecs.filter(
          (r) =>
            r.channel === AppealChannel.ORAL &&
            r.recipient?.includes("head") &&
            !r.is_field_meeting,
        ).length,
        oral_field: roRecs.filter(
          (r) =>
            r.channel === AppealChannel.ORAL &&
            r.recipient?.includes("head") &&
            r.is_field_meeting,
        ).length,
        oral_staff: roRecs.filter(
          (r) =>
        ...
        referral_related: roRecs.filter((r) => r.status === AppealStatus.ROUTED)
          .length,
        being_considered: roRecs.filter(
          (r) => r.status === AppealStatus.BEING_CONSIDERED,
        ).length,
        vm_prev: roPrevRecs.filter(
          (r) => r.channel === AppealChannel.VIRTUAL_RECEPTION,
        ).length,
        vm_curr: roRecs.filter(
          (r) => r.channel === AppealChannel.VIRTUAL_RECEPTION,
        ).length,
        field_meetings_prev: roPrevRecs.filter((r) => r.is_field_meeting)
          .length,
    ...
    regionalOrgs.forEach((ro) => {
      ...
      subjects.forEach((s) => {
        ...
        const sPrevRecs = prevRecords.filter(
          (r) =>
            r.subject_key === s.key &&
            (r.organization?.id === ro.id ||
              (isMain &&
    ...
    regionalOrgs.forEach((ro) => {
      ...
      const getMetrics = (recs: any[]) => ({
        ...
        shikoyat: recs.filter((r) => r.appeal_type === AppealType.SHIKOYAT)
          .length,
      ...
      table5.regional[ro.id] = {
        ...
        phys: {
          curr: getMetrics(
            roRecs.filter((r) => r.applicant_type === ApplicantType.PHYSICAL),
          ),
          prev: getMetrics(
            roPrevRecs.filter(
              (r) => r.applicant_type === ApplicantType.PHYSICAL,
        ...
        legal: {
          curr: getMetrics(
            roRecs.filter((r) => r.applicant_type === ApplicantType.LEGAL),
          ),
          prev: getMetrics(
            roPrevRecs.filter((r) => r.applicant_type === ApplicantType.LEGAL),
          ),
        },
    ...
    const getT6FullMetrics = (recs: any[]) => ({
      ...
      referral: recs.filter((r) => r.status === AppealStatus.ROUTED).length,
      rejected: recs.filter((r) => r.status === AppealStatus.REJECTED).length,
      anonymous: 0, // Still placeholder until ANONYMOUS status is added
      being_considered: recs.filter(
        (r) => r.status === AppealStatus.BEING_CONSIDERED,
      ).length,
      overdue: recs.filter((r) => r.is_overdue).length,
    });

    const peopleRecs = records.filter(
      (r) => r.channel === AppealChannel.PEOPLES_RECEPTION,
    );
    const peopleRecsPrev = prevRecords.filter(
      (r) => r.channel === AppealChannel.PEOPLES_RECEPTION,
    );
    const virtualRecs = records.filter(
      (r) => r.channel === AppealChannel.VIRTUAL_RECEPTION,
    );
    const virtualRecsPrev = prevRecords.filter(
      (r) => r.channel === AppealChannel.VIRTUAL_RECEPTION,
    );
    ...
    // 7. Table 7 Aggregation (Disciplinary Detail Grid YoY - Regional)
    const getT7Metrics = (recs: any[], prevRecs: any[]) => ({
      disciplinary: {
        fine: {
          curr: recs.filter((r) => r.consequence === DisciplinaryMeasure.FINE)
            .length,
          prev: prevRecs.filter(
            (r) => r.consequence === DisciplinaryMeasure.FINE,
          ).length,
        },
        reprimand: {
          curr: recs.filter(
            (r) => r.consequence === DisciplinaryMeasure.REPRIMAND,
          ).length,
          prev: prevRecs.filter(
            (r) => r.consequence === DisciplinaryMeasure.REPRIMAND,
          ).length,
        ...
        dismissal: {
          curr: recs.filter(
            (r) => r.consequence === DisciplinaryMeasure.DISMISSAL,
          ).length,
          prev: prevRecs.filter(
            (r) => r.consequence === DisciplinaryMeasure.DISMISSAL,
          ).length,
        ...
        total: {
          curr: recs.filter((r) =>
            [
              DisciplinaryMeasure.FINE,
              DisciplinaryMeasure.REPRIMAND,
              DisciplinaryMeasure.DISMISSAL,
            ].includes(r.consequence),
          ).length,
          prev: prevRecs.filter((r) =>
            [
              DisciplinaryMeasure.FINE,
              DisciplinaryMeasure.REPRIMAND,
              DisciplinaryMeasure.DISMISSAL,
      ...
      administrative: {
        curr: recs.filter(
          (r) => r.consequence === DisciplinaryMeasure.ADMINISTRATIVE,
        ).length,
        prev: prevRecs.filter(
          (r) => r.consequence === DisciplinaryMeasure.ADMINISTRATIVE,
        ).length,
      ...
      grand_total: {
        curr: recs.filter(
          (r) => r.consequence && r.consequence !== DisciplinaryMeasure.NONE,
        ).length,
        prev: prevRecs.filter(
          (r) => r.consequence && r.consequence !== DisciplinaryMeasure.NONE,
        ).length,
    ...
    const table7: any = {
      summary: getT7Metrics(records, prevRecords),
      regional: {},
    };
    ...
    regionalOrgs.forEach((ro) => {
      ...
      table7.regional[ro.id] = {
        name: ro.name,
        ...getT7Metrics(roRecs, roPrevRecs),
      };
    });
  ...
  async exportPdf(res: Response, organizationId: string, month: string) {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Appeals_Report_${month}.pdf`,
    );
    ...
    doc
      .fontSize(12)
      .text("1-Jadval: Rahbarlar tomonidan murojaatlar ko'rib chiqilishi", {
        underline: true,
      });
    doc.moveDown(0.5);
    const t1 = reports.table1;
    ...
    const renderT1Line = (label: string, data: any) => {
      ...
      doc
        ...
        .font("Helvetica")
        .text(
          `  Jami: ${prevYear}: ${data.total_prev} | ${currYear}: ${data.total_curr}`,
        );
      doc.text(
        `  Og'zaki: ${prevYear}: ${data.oral_prev} | ${currYear}: ${data.oral_curr}`,
      );
      doc.text(
        `  Yozma: ${prevYear}: ${data.written_prev} | ${currYear}: ${data.written_curr}`,
      );
      doc.text(
        `  Elektron: ${prevYear}: ${data.electronic_prev} | ${currYear}: ${data.electronic_curr}`,
      );
      doc.moveDown(0.5);
    };
    ...
    doc
      .fontSize(12)
      .font("Helvetica-Bold")
      .text("2-Jadval: Murojaatlar ijrosi va nazorati (Masalalar)", {
        underline: true,
      });
    doc.moveDown(0.5);
    const t2 = reports.table2;
    Object.entries(t2).forEach(([key, val]: [string, any]) => {
      ...
      doc
        ...
        .font("Helvetica")
        .text(
          `  Jami: ${prevYear}: ${val.total_prev} | ${currYear}: ${val.total_curr}`,
        );
      doc.text(
        `  Yozma: ${prevYear}: ${val.written_prev} | ${currYear}: ${val.written_curr}`,
      );
      doc.text(
        `  Elektron: ${prevYear}: ${val.electronic_prev} | ${currYear}: ${val.electronic_curr}`,
      );
      doc.text(
        `  Og'zaki: ${prevYear}: ${val.oral_prev} | ${currYear}: ${val.oral_curr}`,
      );
      doc.text(
        `  Natijalar (2026): Nazoratda: ${val.under_control}, Chora: ${val.measures_taken}, Tushun.: ${val.explained}, Rad: ${val.rejected}`,
      );
      doc.moveDown(0.5);
    });
    doc.moveDown();
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .text(
        "3-Jadval: Murojaatlarning viloyat bo'yicha tahlili (23-Column Layout)",
        { underline: true },
      );
    doc.moveDown(0.5);
    const t3 = reports.table3;
    Object.values(t3.regional).forEach((reg: any) => {
      ...
      doc
        ...
        .font("Helvetica")
        .text(
          `  Jami: ${prevYear}: ${reg.count_prev} | ${currYear}: ${reg.count_curr}`,
        );
      doc.text(
        `  Toifa (2026): Jismoniy: ${reg.phys_curr}, Yuridik: ${reg.legal_curr}`,
      );
      doc.text(
        `  Shakli (2026): Yozma: ${reg.written}, Elektron: ${reg.electronic}, Og'zaki (Jami): ${reg.oral_total}`,
      );
      doc.text(
    ...
    doc
      .fontSize(12)
      .font("Helvetica-Bold")
      .text("4-Jadval: Murojaat mazmuni (Mavzular)", { underline: true });
    doc.moveDown(0.5);
    const t4 = reports.table4;
    subjects.forEach((s) => {
      ...
      doc
        ...
        .font("Helvetica")
        .text(
          `${s.key}: ${prevYear}: ${countPrev} | ${currYear}: ${countCurr}`,
        );
    });
    doc.moveDown();
    ...
    doc
      .fontSize(12)
      .font("Helvetica-Bold")
      .text("6-Jadval: Xalq va Virtual qabulxonalar", { underline: true });
    doc.moveDown(0.5);
    const t6 = reports.table6;
    doc
      .fontSize(10)
      .font("Helvetica")
      .text(
        `Xalq qabulxonasi: Jami ${prevYear}: ${t6.people.prev.total || 0}, ${currYear}: ${t6.people.curr.total || 0}`,
      );
    doc.text(
      `  (2026 natijalari: Qanoat: ${t6.people.curr.satisfied || 0}, Tushun: ${t6.people.curr.explained || 0}, Rad: ${t6.people.curr.rejected || 0})`,
    );
    doc.text(
      `Virtual qabulxona: Jami ${prevYear}: ${t6.virtual.prev.total || 0}, ${currYear}: ${t6.virtual.curr.total || 0}`,
    );
    doc.text(
      `  (2026 natijalari: Qanoat: ${t6.virtual.curr.satisfied || 0}, Tushun: ${t6.virtual.curr.explained || 0}, Rad: ${t6.virtual.curr.rejected || 0})`,
    );
    doc.moveDown();
    ...
    doc
      .fontSize(12)
      .font("Helvetica-Bold")
      .text("7-Jadval: Intizomiy choralar", { underline: true });
    doc.moveDown(0.5);
    const t7 = reports.table7;
