...
export class KommunalHygieneExportService {
  ...
  async exportRegional(
    ...
    const fileName =
      organizationId && organizationId !== "all" && targetOrgs.length > 0
  ...
  private async buildTable1Sheet(
    wb: ExcelJS.Workbook,
    month: string,
    orgs: any[],
  ) {
    ...
    let waterData = [];
    if (orgIds.length > 0) {
      waterData = await this.waterRepo
    ...
    const rowTypes = [
      ...
      {
        key: "kommunal_norm",
        label: "Shundan sanitariya normalariga javob bermaydi",
      },
      { key: "departmental", label: "Idoraviy vodoprovod" },
      {
        key: "departmental_norm",
        label: "Shundan sanitariya normalariga javob bermaydi",
      },
    ];
    ...
    const fields = [
      "chem_total",
      "chem_src_manba",
      "chem_src_tarmok_oldin",
      "chem_src_tarmok_point",
      "chem_src_consumer",
      "chem_bad_ammiak",
      "chem_bad_nitrat",
      "chem_bad_nitrit",
      ...
      "bact_src_manba",
      "bact_src_tarmok_oldin",
      "bact_src_tarmok_point",
      "bact_src_consumer",
      "bact_bad_umc",
      "bact_bad_koli",
      "bact_bad_sfz",
    ...
    fields.forEach((f, idx) => {
      jamiKI.push(
        Number(exportRows[0][idx + 1]) + Number(exportRows[2][idx + 1]),
      );
      jamiTKD.push(
        Number(exportRows[1][idx + 1]) + Number(exportRows[3][idx + 1]),
      );
    });
  ...
  private async buildTable2Sheet(
    wb: ExcelJS.Workbook,
    month: string,
    orgs: any[],
  ) {
    ...
    const subHdrs = [
      [5, "Olingan namunalar soni"],
      [6, "Javob bermaydi"],
      [7, "Olingan namunalar soni"],
      [8, "Javob bermaydi"],
      [10, "Olingan namunalar soni"],
      [11, "Javob bermaydi"],
      [12, "Olingan namunalar soni"],
      [13, "Javob bermaydi"],
    ];
    subHdrs.forEach(([c, v]) => {
    ...
    data.forEach((r) => {
      ...
      const chem_eff =
        r.chem_before_bad > 0
          ? (
              ((r.chem_before_bad - r.chem_after_bad) / r.chem_before_bad) *
              100
            ).toFixed(1)
          : "0.0";
      const bact_eff =
        r.bact_before_bad > 0
          ? (
              ((r.bact_before_bad - r.bact_after_bad) / r.bact_before_bad) *
              100
            ).toFixed(1)
          : "0.0";
    ...
    const globalChemEff =
      totalTotals.cbb > 0
        ? (
            ((totalTotals.cbb - totalTotals.cab) / totalTotals.cbb) *
            100
          ).toFixed(1)
        : "0.0";
    const globalBactEff =
      totalTotals.bbb > 0
        ? (
            ((totalTotals.bbb - totalTotals.bab) / totalTotals.bbb) *
            100
          ).toFixed(1)
        : "0.0";
  ...
  private async buildTable3Sheet(
    wb: ExcelJS.Workbook,
    month: string,
    orgs: any[],
  ) {
    ...
    let data = [];
    if (orgIds.length > 0) {
      data = await this.waterUsageRepo
        .createQueryBuilder("r")
        .leftJoinAndSelect("r.organization", "org")
        .where("r.reportMonth = :m", { m: month })
        .andWhere("org.id IN (:...orgIds)", { orgIds })
    ...
    ws.mergeCells("D1:L1");
    ws.getCell("D1").value =
      "Ochiq suv havzalarining ruxsat etilgan me'yorlaridan chetlashishi";
    this.setupHeaderStyle(ws.getCell("D1"));
    ...
    ws.mergeCells("E2:E3");
    ws.getCell("E2").value =
      "Sanitariya normalariga javob bermaydigan namunalar (bakt)";
    this.setupHeaderStyle(ws.getCell("E2"));
    ...
    ws.mergeCells("I2:J2");
    ws.getCell("I2").value =
      "Kimyoviy ko'rsatkichlar tahlili uchun jami namunalar, shu jumladan zaharli ximikatlar";
    this.setupHeaderStyle(ws.getCell("I2"));
    ws.getCell("I3").value = "Jami";
    this.setupHeaderStyle(ws.getCell("I3"));
    ws.getCell("J3").value = "Shu jumladan zaharli ximikatlar";
    this.setupHeaderStyle(ws.getCell("J3"));

    ws.mergeCells("K2:L2");
    ws.getCell("K2").value =
      "Sanitariya ko'rsatkichlariga javob bermaydigan namunalar (kimyo, shu jumladan zaxarli ximikatlar)";
    this.setupHeaderStyle(ws.getCell("K2"));
    ws.getCell("K3").value = "Jami";
    this.setupHeaderStyle(ws.getCell("K3"));
    ws.getCell("L3").value = "Shu jumladan zaharli ximikatlar javob bermaydi";
    this.setupHeaderStyle(ws.getCell("L3"));
    ...
    data.forEach((r) => {
      ...
      const row = ws.addRow([
        ...
        r.samples_taken,
        r.samples_bad,
        r.pathogen_inf_disease,
        r.pathogen_cholera,
        r.pathogen_parasite,
        r.chem_samples_total,
        r.chem_pesticide_presence,
        r.chem_bad_total,
        r.chem_bad_pesticide,
      ]);
      row.eachCell((cell) => this.setupDataStyle(cell));
    });
