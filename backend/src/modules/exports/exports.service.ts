...
export class ExportsService {
  ...
  async exportForm1ToExcel(res: Response, startDate: string, endDate: string, includeTest = false, user: User) {
    const reports = await this.getForm1Reports(startDate, endDate, includeTest, user);
    const workbook = new ExcelJS.Workbook();
    ...
    const setStyle = (cellOrRange: string, fillHex?: string) => {
      ...
      cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } };
      if (fillHex) {
        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF' + fillHex.replace('#', '') } };
      }
    };

    // Row 2
    worksheet.mergeCells('A2:A4'); setStyle('A2'); worksheet.getCell('A2').value = "Ko'rsatkichlar nomi";
    worksheet.mergeCells('B2:B4'); setStyle('B2'); worksheet.getCell('B2').value = "Kod";

    // Row 2 Group
    worksheet.mergeCells('C2:N2'); setStyle('C2'); worksheet.getCell('C2').value = "Joriy oy";
    worksheet.mergeCells('O2:Z2'); setStyle('O2'); worksheet.getCell('O2').value = "Yil boshidan jami";

    // Row 3
    worksheet.mergeCells('C3:H3'); setStyle('C3'); worksheet.getCell('C3').value = "Jami";
    worksheet.mergeCells('I3:N3'); setStyle('I3'); worksheet.getCell('I3').value = "14 yoshgacha";
    worksheet.mergeCells('O3:T3'); setStyle('O3'); worksheet.getCell('O3').value = "Jami";
    worksheet.mergeCells('U3:Z3'); setStyle('U3'); worksheet.getCell('U3').value = "14 yoshgacha";

    // Row 4 (Years)
    worksheet.mergeCells('C4:D4'); setStyle('C4', 'b7eb8f'); worksheet.getCell('C4').value = `${prevYear} yil`;
    worksheet.mergeCells('E4:F4'); setStyle('E4', 'fffb8f'); worksheet.getCell('E4').value = `${currentYear} yil`;
    worksheet.mergeCells('G4:H4'); setStyle('G4'); worksheet.getCell('G4').value = "ko'tar/pasayish";

    worksheet.mergeCells('I4:J4'); setStyle('I4', 'b7eb8f'); worksheet.getCell('I4').value = `${prevYear} yil`;
    worksheet.mergeCells('K4:L4'); setStyle('K4', 'fffb8f'); worksheet.getCell('K4').value = `${currentYear} yil`;
    worksheet.mergeCells('M4:N4'); setStyle('M4'); worksheet.getCell('M4').value = "ko'tar/pasayish";

    worksheet.mergeCells('O4:P4'); setStyle('O4', 'b7eb8f'); worksheet.getCell('O4').value = `${prevYear} yil`;
    worksheet.mergeCells('Q4:R4'); setStyle('Q4', 'fffb8f'); worksheet.getCell('Q4').value = `${currentYear} yil`;
    worksheet.mergeCells('S4:T4'); setStyle('S4'); worksheet.getCell('S4').value = "ko'tar/pasayish";

    worksheet.mergeCells('U4:V4'); setStyle('U4', 'b7eb8f'); worksheet.getCell('U4').value = `${prevYear} yil`;
    worksheet.mergeCells('W4:X4'); setStyle('W4', 'fffb8f'); worksheet.getCell('W4').value = `${currentYear} yil`;
    worksheet.mergeCells('Y4:Z4'); setStyle('Y4'); worksheet.getCell('Y4').value = "ko'tar/pasayish";

    // Row 5 (Sub headers)
    const headerCols = [
      'abs.ko\'r', 'int.ko\'r', // Monthly - Jami - Prev
      'abs.ko\'r', 'int.ko\'r', // Monthly - Jami - Curr
      'abs.ko\'r', 'int.ko\'r', // Monthly - Jami - Growth
      'abs.ko\'r', 'int.ko\'r', // Monthly - U14 - Prev
      'abs.ko\'r', 'int.ko\'r', // Monthly - U14 - Curr
      'abs.ko\'r', 'int.ko\'r', // Monthly - U14 - Growth
      'abs.ko\'r', 'int.ko\'r', // YTD - Jami - Prev
      'abs.ko\'r', 'int.ko\'r', // YTD - Jami - Curr
      'abs.ko\'r', 'int.ko\'r', // YTD - Jami - Growth
      'abs.ko\'r', 'int.ko\'r', // YTD - U14 - Prev
      'abs.ko\'r', 'int.ko\'r', // YTD - U14 - Curr
      'abs.ko\'r', 'int.ko\'r'  // YTD - U14 - Growth
    ];
    ...
    headerCols.forEach((val, idx) => {
      ...
      if ([0, 1, 6, 7, 12, 13, 18, 19].includes(idx)) {
        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFb7eb8f' } };
      } else if ([2, 3, 8, 9, 14, 15, 20, 21].includes(idx)) {
        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFfffb8f' } };
      }
    });
    ...
    allDiseases.forEach(d => {
      aggregatedData[d.code] = {
        code: d.code,
        name: d.name,
        m_t_p_a: 0, m_t_p_i: 0, m_t_c_a: 0, m_t_c_i: 0,
        m_u_p_a: 0, m_u_p_i: 0, m_u_c_a: 0, m_u_c_i: 0,
        y_t_p_a: 0, y_t_p_i: 0, y_t_c_a: 0, y_t_c_i: 0,
        y_u_p_a: 0, y_u_p_i: 0, y_u_c_a: 0, y_u_c_i: 0
      };
    });
    ...
    const sortedRows = Object.values(aggregatedData).sort((a, b) => Number(a.code) - Number(b.code));
    ...
    sortedRows.forEach(d => {
      ...
      row.values = [
        ...
        d.m_t_p_a, d.m_t_p_i,
        d.m_t_c_a, d.m_t_c_i,
        d.m_t_c_a - d.m_t_p_a, 0, // Growth (Abs, Placeholder for Int)
        // Monthly - U14
        d.m_u_p_a, d.m_u_p_i,
        d.m_u_c_a, d.m_u_c_i,
        d.m_u_c_a - d.m_u_p_a, 0,
        // YTD - Total
        d.y_t_p_a, d.y_t_p_i,
        d.y_t_c_a, d.y_t_c_i,
        d.y_t_c_a - d.y_t_p_a, 0,
        // YTD - U14
        d.y_u_p_a, d.y_u_p_i,
        d.y_u_c_a, d.y_u_c_i,
        d.y_u_c_a - d.y_u_p_a, 0
      ];
    ...
    const orgs = await this.orgRepo.find({ where: { parent: Not(IsNull()) }, order: { name: 'ASC' } });
    ...
    sortedRows.forEach((diseaseObj: any) => {
      ...
      currentRow++;
      worksheet2.mergeCells(`A${currentRow}:A${currentRow + 3}`); setStyle2(`A${currentRow}`); worksheet2.getCell(`A${currentRow}`).value = "Administrativnye territorii";
      worksheet2.mergeCells(`B${currentRow}:B${currentRow + 3}`); setStyle2(`B${currentRow}`); worksheet2.getCell(`B${currentRow}`).value = `Stroka ${code}`;

      worksheet2.mergeCells(`C${currentRow}:N${currentRow}`); setStyle2(`C${currentRow}`); worksheet2.getCell(`C${currentRow}`).value = "tekushiy oy (joriy oy)";
      worksheet2.mergeCells(`O${currentRow}:Z${currentRow}`); setStyle2(`O${currentRow}`); worksheet2.getCell(`O${currentRow}`).value = "narastayushiy itog (yil boshidan)";

      currentRow++;
      worksheet2.mergeCells(`C${currentRow}:H${currentRow}`); setStyle2(`C${currentRow}`); worksheet2.getCell(`C${currentRow}`).value = "Jami";
      worksheet2.mergeCells(`I${currentRow}:N${currentRow}`); setStyle2(`I${currentRow}`); worksheet2.getCell(`I${currentRow}`).value = "deti do 14 let (bolalar)";
      worksheet2.mergeCells(`O${currentRow}:T${currentRow}`); setStyle2(`O${currentRow}`); worksheet2.getCell(`O${currentRow}`).value = "Jami";
      worksheet2.mergeCells(`U${currentRow}:Z${currentRow}`); setStyle2(`U${currentRow}`); worksheet2.getCell(`U${currentRow}`).value = "deti do 14 let (bolalar)";

      currentRow++;
      const years = [prevYear, currentYear, prevYear, currentYear, prevYear, currentYear, prevYear, currentYear];
      const startCols = [3, 5, 9, 11, 15, 17, 21, 23];
      years.forEach((y, i) => {
        const col = startCols[i];
        const isCurr = i % 2 !== 0;
        worksheet2.mergeCells(currentRow, col, currentRow, col + 1);
        setStyle2(worksheet2.getCell(currentRow, col).address, isCurr ? 'fffb8f' : 'b7eb8f');
      ...
      orgs.forEach(org => {
        ...
        row.values = [
          org.name, code,
          d.m_t_p_a || 0, d.m_t_p_i || 0, d.m_t_c_a || 0, d.m_t_c_i || 0, (d.m_t_c_a || 0) - (d.m_t_p_a || 0), 0,
          d.m_u_p_a || 0, d.m_u_p_i || 0, d.m_u_c_a || 0, d.m_u_c_i || 0, (d.m_u_c_a || 0) - (d.m_u_p_a || 0), 0,
          d.y_t_p_a || 0, d.y_t_p_i || 0, d.y_t_c_a || 0, d.y_t_c_i || 0, (d.y_t_c_a || 0) - (d.y_t_p_a || 0), 0,
          d.y_u_p_a || 0, d.y_u_p_i || 0, d.y_u_c_a || 0, d.y_u_c_i || 0, (d.y_u_c_a || 0) - (d.y_u_p_a || 0), 0
        ];

        row.eachCell((cell, colNumber) => {
          cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } };
          cell.font = { name: 'Times New Roman', size: 9 };
          if ([3, 4, 9, 10, 15, 16, 21, 22].includes(colNumber)) cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFb7eb8f' } };
          if ([5, 6, 11, 12, 17, 18, 23, 24].includes(colNumber)) cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFfffb8f' } };
        });
        currentRow++;
      });
    ...
    const majorDiseases = allDiseases.filter(d => majorCodes.includes(d.code)).sort((a, b) => majorCodes.indexOf(a.code) - majorCodes.indexOf(b.code));
    ...
    orgs.forEach((org, orgIdx) => {
      ...
      majorDiseases.forEach((d, dIdx) => {
        ...
        [diseaseData.m_t_c_a || 0, diseaseData.m_t_c_i || 0].forEach((val, vIdx) => {
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename=Form1_Report_${startDate}.xlsx`);
    await workbook.xlsx.write(res);
  }
}
