...
import { NutritionActionRecord, NutritionObjectType, NutritionEntryType, NutritionMeasure, LabResult } from "./entities/nutrition-action-record.entity";
...
export class NutritionHygieneService {
    ...
    async generateReportsFromRecords(organizationId: string, month: string) {
        ...
        const table1 = {
            production_notif: records.filter(r => r.object_type === NutritionObjectType.PRODUCTION && r.entry_type === NutritionEntryType.INSPECTION).length,
            catering_notif: records.filter(r => r.object_type === NutritionObjectType.CATERING && r.entry_type === NutritionEntryType.INSPECTION).length,
            trade_notif: records.filter(r => r.object_type === NutritionObjectType.TRADE && r.entry_type === NutritionEntryType.INSPECTION).length,
            sanitary_fine_count: records.filter(r => r.measure_type === NutritionMeasure.FINE).length,
            sanitary_fine_sum: records.filter(r => r.measure_type === NutritionMeasure.FINE).reduce((sum, r) => sum + Number(r.fine_sum), 0),
            suspension_count: records.filter(r => r.measure_type === NutritionMeasure.STOP_OPERATION).length,
        };
        ...
        const table2 = {
            ...
            dismissal_proposals: records.filter(r => r.measure_type === NutritionMeasure.DISMISSAL_PROPOSAL).length,
            dismissed_employees: records.filter(r => r.measure_type === NutritionMeasure.DISMISSAL_ACTUAL).length,
            medical_checked_after_proposal: 0,
            protocols_count: records.filter(r => r.measure_type === NutritionMeasure.PROTOCOL).length,
        };

        // T3: Product Categories
        const categories = ["meat_products", "milk_products", "fish_products", "bread_products", "sugar_products", "fruit_veg", "fat_oil", "alcohol_soft", "baby_food", "canned_food", "salt", "other"];
        const table3: any = {};
        categories.forEach(cat => {
            table3[cat] = records.filter(r => r.product_category === cat).reduce((sum, r) => sum + Number(r.amount), 0);
        });
        table3["total_samples"] = records.filter(r => r.entry_type === NutritionEntryType.LAB_SAMPLE).length;

        // T4 & T5: Salt & Flour
        const table4 = {
            potassium_iodate_kg: records.filter(r => r.object_type === NutritionObjectType.ENT_SALT).reduce((sum, r) => sum + Number(r.amount), 0),
            samples_prod_total: records.filter(r => r.object_type === NutritionObjectType.ENT_SALT).length,
        };

        const table5 = {
            premix_amount_kg: records.filter(r => r.object_type === NutritionObjectType.ENT_FLOUR).reduce((sum, r) => sum + Number(r.amount), 0),
            samples_prod_total: records.filter(r => r.object_type === NutritionObjectType.ENT_FLOUR).length,
        };

        // T6: Markets
        const table6 = {
            inspections_total: records.filter(r => r.object_type === NutritionObjectType.MARKET).length,
            fine_individual_count: records.filter(r => r.object_type === NutritionObjectType.MARKET && r.measure_type === NutritionMeasure.FINE).length,
            fine_individual_sum: records.filter(r => r.object_type === NutritionObjectType.MARKET && r.measure_type === NutritionMeasure.FINE).reduce((sum, r) => sum + Number(r.fine_sum), 0),
        };
