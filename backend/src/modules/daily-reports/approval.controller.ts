import { Controller, Patch, Param, UseGuards, Request, Get, Query } from '@nestjs/common';
...
export class ApprovalController {
    ...
    async verifyReport(@Param('type') type: string, @Param('id') id: string, @Request() req) {
        const user = req.user;
        // Check if user is Dept Head or Admin
        if (user.role !== UserRole.DEPARTMENT_HEAD && user.role !== UserRole.ADMIN && user.role !== UserRole.LAB_HEAD) {
    ...
    async approveReport(@Param('type') type: string, @Param('id') id: string, @Request() req) {
        const user = req.user;
    ...
    private getTableName(type: string): string {
        switch (type.toLowerCase()) {
            case 'hepatitis': return 'hepatitis_daily_reports';
            case 'flu': return 'flu_daily_reports';
            case 'ari': return 'ari_daily_reports';
            case 'covid': return 'covid_daily_reports';
            case 'epidemiology': return 'epidemiology_daily_reports';
            default: throw new Error('Yaroqsiz hisobot turi');
        }
    }
}
