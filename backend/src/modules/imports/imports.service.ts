...
export class ImportsService {
    ...
    async processImport(file: Express.Multer.File, type: string) {
        try {
            ...
            throw new BadRequestException("Faylni o'qishda xatolik: " + error.message);
        }
    }

    private async upsertHepatitis(row: any, organization: Organization, reportDate: string) {
    ...
    private async upsertFlu(row: any, organization: Organization, reportDate: string) {
        const entityData = {
            reportDate: reportDate,
            organization: { id: organization.id },
            total_ari: Number(row['Jami O\'RVI'] || row['total_ari'] || 0),
            pneumonia_total: Number(row['Pnevmoniya Jami'] || row['pneumonia_total'] || 0),
    ...
    private async upsertForm1(row: any, organization: Organization, reportDate: string) {
        ...
        let template = await this.templateRepo.findOne({ where: { code: 'form_1' } });
