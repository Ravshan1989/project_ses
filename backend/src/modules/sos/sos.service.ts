...
import { SosAlert, SosStatus, SosReviewStatus } from './entities/sos-alert.entity';
...
export class SosService {
    ...
    async createPredefinedDisease(dto: CreateSosDiseaseDto, user: User) {
        if (user.role !== UserRole.ADMIN) {
            throw new ForbiddenException('Faqat Admin kasalliklar ro\'yxatini boshqara oladi.');
        }
    ...
    async createAlert(dto: CreateSosAlertDto, user: User) {
        if (!user.organization) {
            throw new ForbiddenException('Foydalanuvchi tashkilotga biriktirilmagan.');
        }
        ...
        await this.telegramService.sendSosNotification({
            ...
            status: saved.status === SosStatus.CONFIRMED ? 'Aniqlangan' : 'Gumon qilinmoqda',
    ...
    async getAlerts(user: User) {
        ...
        if (level === 2 && user.organization) {
            return this.alertRepo.createQueryBuilder('alert')
                ...
                .where('organization.parent_id = :orgId OR organization.id = :orgId', { orgId: user.organization.id })
    ...
    async markAsReviewed(id: string, user: User) {
        const level = getRoleLevel(user.role);
        if (level > 2) {
            throw new ForbiddenException('Faqat Viloyat yoki Respublika darajasidagi xodimlar tasdiqlashi mumkin.');
        }
