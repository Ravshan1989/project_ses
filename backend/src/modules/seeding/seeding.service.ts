...
export class SeedingService implements OnModuleInit {
    ...
    private async seedDepartmentPermissions() {
        const permissionsData = [
            ...
            { code: "VIEW_SANITARY", description: "View Sanitary Reports (Tekshirishlar)" },
        ...
        const departmentsData = [
            ...
            { name: "Sanitariya Bo'limi", description: "Faqat sanitariya tadbirlarini ko'radi", level: 1 },
            { name: "Epidemiologiya va immunoprofilaktika", description: "Tuman darajasi: Faqat Forma 1-jadval va kunliklar", level: 3 },
            { name: "Sanitariya", description: "Tuman darajasi: Sanitariya va gigiyena tadbirlari", level: 3 },
        ];
        ...
        const level1Assignments = [
            ...
            },
            {
                ...
                perms: [
                    "VIEW_HEPATITIS", "VIEW_FLU", "VIEW_EPIDEMIOLOGY", "VIEW_WEEKLY_SUMMARY",
                    "VIEW_COVID", "VIEW_ARI", "VIEW_DIARRHEA", "VIEW_SANITARY",
                    "VIEW_FORM1_TABLE1", "VIEW_FORM1_TABLE2", "VIEW_FORM1_TABLE3",
                    "EDIT_FORM1_TABLE1", "VERIFY_REPORT", "APPROVE_REPORT", "MANAGE_DEPARTMENTS",
                ],
            },
        ];
        ...
        const level3Perms = [
            "VIEW_FORM1_TABLE1", "EDIT_FORM1_TABLE1", "VIEW_HEPATITIS", "VIEW_FLU",
            "VIEW_EPIDEMIOLOGY", "VIEW_WEEKLY_SUMMARY", "VIEW_COVID", "VIEW_ARI",
            "VIEW_DIARRHEA", "VIEW_SANITARY",
        ];
    ...
    private async assignPermissionToDept(dept: Department, code: string) {
        ...
        if (!exists) {
            await this.deptPermRepo.save(this.deptPermRepo.create({ department: dept, permission: perm }));
        }
    }
    ...
    private async seedPermissions() {
        const rolesToSeed = [
            ...
            { name: UserRole.SANITARY_HEAD, level: 3, description: "Sanitariya Bo'limi Mudiri" },
            { name: UserRole.SANITARY_SPECIALIST, level: 3, description: 'Sanitar Vrach' },
            { name: UserRole.SANITARY_OPERATOR, level: 3, description: 'Sanitar Yordamchisi' },
        ];
        ...
        const assignments = [
            {
                ...
                canCreate: true, canEdit: false, canApprove: false, canDownload: true,
            },
            {
                role: UserRole.DISTRICT_SPECIALIST,
                perms: ['VIEW_EPIDEMIOLOGY', 'VIEW_HEPATITIS', 'VIEW_FLU', 'VIEW_ARI', 'VIEW_COVID', 'VIEW_DIARRHEA'],
                canCreate: true, canEdit: true, canApprove: false, canDownload: true,
            },
            {
                role: UserRole.DEPARTMENT_HEAD,
                perms: ['VIEW_EPIDEMIOLOGY', 'VIEW_HEPATITIS', 'VIEW_FLU', 'VIEW_ARI', 'VIEW_COVID', 'VIEW_DIARRHEA', 'VERIFY_REPORT'],
                canCreate: false, canEdit: false, canApprove: true, canDownload: true,
            },
            {
        ...
        for (const assign of assignments) {
            ...
            for (const code of assign.perms) {
                await this.rolePermRepo.save(this.rolePermRepo.create({
                    role: role, permissionCode: code, canView: true,
                    canCreate: assign.canCreate, canEdit: assign.canEdit,
                    canApprove: assign.canApprove, canDownload: assign.canDownload,
                }));
            }
        }
    }
        ...
        for (const u of users) {
            ...
            if (!exists) {
                await this.usersService.create({
                    username: u.username, passwordHash: hash, role: u.role,
                    dynamicRoleId: roleEntity?.id, organizationId: chirchiq.id,
                    departmentId: dept.id, firstName: u.name, lastName: 'Test',
                });
