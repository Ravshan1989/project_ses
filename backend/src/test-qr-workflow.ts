...
async function testQRWorkflow() {
  ...
  const fluRepo: Repository<FluDailyReport> = app.get(
    getRepositoryToken(FluDailyReport),
  );
  ...
  try {
    ...
    const districtUser =
      await usersService.findOneByUsername("user_olmaliq_sh");
    ...
    let report = await fluRepo.findOne({
      where: {
        reportDate: today,
        organization: { id: districtUser.organization.id },
        isTest: true,
      },
    });
    ...
    const verifyResult = await approvalController.verifyReport(
      "flu",
      report.id,
      mockReq,
    );
    ...
    console.log(`   Status: ${report.status}`);
    console.log(
      `   QR Token: ${report.verificationToken ? "✅ YARATILDI!" : "❌ YO'Q"}`,
    );
    ...
    console.log(`   Status: ${report.status}`);
    console.log(
      `   QR Token: ${report.verificationToken ? "✅ SAQLANIB QOLDI!" : "❌ YO'Q"}`,
    );
    console.log(`   Token qiymati: ${report.verificationToken}\n`);
    ...
    console.log(
      `✅ QR link: ${process.env.FRONTEND_URL || "http://localhost:5173"}/verify/${report.verificationToken}`,
    );
  } catch (err) {
