...
async function bootstrap() {
    ...
    const tableBody = accounts.map(a => `| ${a.Organization} | \`${a.Username}\` | \`${a.Password}\` |`).join("\n");
    fs.writeFileSync("all_users_credentials.md", tableHeader + tableBody);

    console.log("Seeding all users complete. Credentials saved to all_users_credentials.md");
    await app.close();
}
