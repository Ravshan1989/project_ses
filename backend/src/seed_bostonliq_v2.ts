...
async function seedBostonliqUsers() {
    const ds = new DataSource({
        ...
        entities: [
            Organization, User, Department, Role, UserPermission,
            DepartmentPermission, RolePermission, Permission,
            Submission, SosAlert, SosDisease, Template, Disease,
            AriDailyReport, CovidDailyReport, DiarrheaDailyReport,
            EpidemiologyDailyReport, FluDailyReport, HepatitisDailyReport
        ],
        synchronize: false,
    });
    ...
    try {
        ...
        const usersToCreate = [
            { username: 'bostonliq_boshliq', firstName: 'Bo\'lim', lastName: 'Boshlig\'i', role: UserRole.DEPARTMENT_HEAD },
            { username: 'bostonliq_mudir', firstName: 'Bo\'lim', lastName: 'Mudiri', role: UserRole.DEPARTMENT_HEAD },
            { username: 'bostonliq_hod1', firstName: 'Hodim', lastName: '1', role: UserRole.STAFF },
            { username: 'bostonliq_hod2', firstName: 'Hodim', lastName: '2', role: UserRole.STAFF },
            { username: 'bostonliq_hod3', firstName: 'Hodim', lastName: '3', role: UserRole.STAFF },
        ];

        for (const u of usersToCreate) {
            const existing = await ds.getRepository(User).findOne({ where: { username: u.username } });
            if (existing) {
