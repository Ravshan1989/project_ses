...
async function verifyInternal() {
  ...
  try {
    ...
    const distData = await reportsService.getWeeklySummary(
      today,
      today,
      districtUser,
      true,
    ...
    if (
      distData.length === 1 &&
      distData[0].organization.name === "Olmaliq sh"
    ) {
      console.log("   SUCCESS: District only sees its own data.");
    } else {
      console.error(
        "   FAILURE: District summary data mismatch!",
        distData.length,
      );
    }
    ...
    const regData = await reportsService.getWeeklySummary(
      today,
      today,
      regionUser,
      true,
    ...
    const olmaliqEntry = regData.find(
      (d) => d.organization.name === "Olmaliq sh",
    );
    if (olmaliqEntry && olmaliqEntry.ari_total === 100) {
      console.log("   SUCCESS: Region sees aggregated data from Olmaliq.");
    } else {
      console.error(
        "   FAILURE: Region summary data mismatch or missing Olmaliq!",
      );
    }
